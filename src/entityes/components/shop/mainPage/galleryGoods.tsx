...
function randomArrayValue(paramArr: Array<unknown>) {
  const value = Math.round(Math.random() * paramArr.length);
  //console.log(value);

  return paramArr[Math.min(value, paramArr.length)];
}
...
const GalleryGoods = memo(({ goods }: { goods: TGoodItem[] }) => {
  ...
  const divClass = useMemo<string>(() => {
    return `mt-5 w-full h-80 rounded-3xl ${randomArrayValue(backgroundsLight) as string} dark:${randomArrayValue(backgroundsDark) as string} py-2 px-3 overflow-hidden relative z-1`;
  }, []);
  ...
  return (
    <div ref={containerRef} className={divClass}>
      {showButtons && (
